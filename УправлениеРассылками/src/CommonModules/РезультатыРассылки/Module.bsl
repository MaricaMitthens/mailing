	...
	Пока УчЗапись.Следующий() Цикл
		...
		Для Индекс=0 по МассивСообщений.Количество() - 1 Цикл
			...
			Если (СтрНайти(НРег(Тема), "отпис")) Тогда
				...
				ТекущийСегмент = Запись.Рассылка.Получатели.ПолучитьОбъект();
				Для Индекс = 0 По ТекущийСегмент.Состав.Количество() - 1 Цикл
					Если (ТекущийСегмент.Состав[Индекс].Контрагент.Наименование=Запись.Контрагент.Наименование) Тогда
						ТекущийСегмент.Состав.Удалить(Индекс);
						Прервать;
					КонецЕсли;
				КонецЦикла;
				ТекущийСегмент.Записать();
