...
Процедура ОтправитьПисьмо(Почта, Выборка)
	...
	Текст = ЗаполнитьПараметрыШаблона(Выборка.Рассылка.Шаблон.ТекстПисьма, Выборка);
	Подпись = ЗаполнитьПараметрыШаблона(Выборка.Рассылка.Шаблон.Подпись, Выборка);
	...
	Сообщение.Тексты.Добавить(Символы.ПС);
	Сообщение.Тексты.Добавить(Подпись, ТипТекстаПочтовогоСообщения.ПростойТекст);
...
Процедура ОтправкаПисемИзОчереди() Экспорт
	...
	Пока ВыборкаУчетнаяЗаписьЭП.Следующий() Цикл
		...
		Если (Профиль = 0) Тогда
			ВнестиДанныеОбОшибке(ВыборкаУчетнаяЗаписьЭП);
			Возврат;
		КонецЕсли;
...
Функция ЗаполнитьПараметрыШаблона(Текст, Выборка)
	НовыйТекст = Строка(Текст);
	НовыйТекст = СтрЗаменить(НовыйТекст, "{ИМЯ}", Выборка.Контрагент.Имя);
	НовыйТекст = СтрЗаменить(НовыйТекст, "{ФАМИЛИЯ}", Выборка.Контрагент.Фамилия);
	НовыйТекст = СтрЗаменить(НовыйТекст, "{ОТЧЕСТВО}", Выборка.Контрагент.Отчество);
	НовыйТекст = СтрЗаменить(НовыйТекст, "{ИМЯОТПРАВИТЕЛЯ}", Выборка.Рассылка.ИмяОтправителя);
	Возврат НовыйТекст;
КонецФункции
