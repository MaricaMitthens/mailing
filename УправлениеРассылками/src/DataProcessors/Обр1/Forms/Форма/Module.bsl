...
Процедура ОтправитьПисьмо(Почта, Выборка)
	...
	ТекстСообщения.Текст = "<a href=""mailto:1cservicesspb@gmail.com?subject=Отписаться от рассылки&body=Отправьте это письмо для отписки от рассылки"">Отправьте для отписки от рассылки</a>";
...
Процедура ВыбратьПисьмаНаСервере()
	...
	Пока ВыборкаУчетнаяЗаписьЭП.Следующий() Цикл
		...
		//TODO: записать в регистр ПротоколОтправки, статус НеОтправлено + описание
		Возврат;
		КонецЕсли;
		Почта = ПодключитьсяКПочте(Профиль);
		Если (Почта=0) Тогда
			//TODO: записать ош. в регистр ПротоколОтправки, статус НеОтправлено + описание ошибки
			Возврат;
		КонецЕсли;
